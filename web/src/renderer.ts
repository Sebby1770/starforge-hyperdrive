...
export function createRenderer(canvas: HTMLCanvasElement): Renderer {
  ...
  return {
    ...
    draw(framebuffer: Uint8ClampedArray, width: number, height: number) {
      // The engine can hand us a differently-sized frame than the last resize()
      // (a reload against a rebuilt WASM, for instance); rebuild the backing
      // ImageData rather than throwing on a set() length mismatch.
